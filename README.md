...
Доступна внешняя обработка [test.epf](https://github.com/WendyH/TenzoM1CAddin/raw/master/test.epf), для примера и тестирования внешней компоненты.
![test.png](https://github.com/WendyH/TenzoM1CAddin/raw/master/test.png)
...
При вызове данной функции обновляются значения свойств `ВесСтабилен`, `Событие`, `ВторойДатчик`, `Перегрузка`.
...
Состояние свойства `Событие` обновляется при считывании веса, вызове функции `ПолучитьВес()`.
И сбрасывается после вызова функции `ПолучитьВведенныйКод()`.

### `ПолучитьТекстИндикатора(НомерУстройства) / GetIndicatorText`
Возвращает текст, отображаемый на индикаторе весового терминала. Работает только по протоколу TenzoM.

Где:<br>
`НомерУстройства` - число, номер устройства вывода. Описывается дополнительно для каждого весового
терминала в документации протокола обмена (код операции: C6h).
- 0x00 (0) - не передавать код номера устройства (для некоторых терминалов не используется)
- 0x01 (1) - основной индикатор (ТВ-003, ТВ-005, ТВ-009)
- 0x02 (2) - дополнительный индикатор (ТВ-003, ТВ-009)
- 0x03 (3) - матричный принтер (что бы это ни значило)
- 0x0F (15) – индикатор ТВ-015
- 0x1F (31) - верхняя строка индикатора (ТВ-014, ТВ-015, ТВ-019)
- 0x20 (32) - нижняя строка индикатора (ТВ-015, ТВ-019)
- 0x21 (33) - нижняя и верхняя строка индикатора (ТВ-015, ТВ-019)<br>
Необязательное. Значчение по-умолчанию = 0x21 (33).

Если номер устройства передан не поддерживаемый - в свойстве `КодОшибки` будет значение 1.
Можно перебрать все значения, сбрасывая код ошибки перед вызовом и проверяя после, если ошибки не возникнет - значит
мы нашли поддерживаемый код устройства вывода (индикатора).   

### `УстановитьТекстИндикатора(Текст, НомерУстройства) / SetIndicatorText`
Выводит текст на индикаторы весового терминала. Работает только по протоколу TenzoM.

Где:<br>
`Текст` - строка, текст, кооторый будет выведен на индикатор. Имеет смысл передавать той длиной, которая поддерживается самим индикатором на устройстве.

`НомерУстройства` - число, номер устройства вывода. Описывается дополнительно для каждого весового
терминала в документации протокола обмена (код операции: D2h).
Этот номер может не совпадать с тем, который используется для вызова функции `ПолучитьТекстИндикатора()`.
- 0x00 (0) - не передавать код номера устройства (для некоторых терминалов не используется)
- 0x20 (32) - нижняя строка индикатора (ТВ-015, ТВ-019)
- 0x21 (33) - верхняя строка индикатора (ТВ-015, ТВ-019)
- 0x22 (34) - верхняя и нижняя строка индикатора (ТВ-015, ТВ-019)<br>
Необязательное. Значение по-умолчанию = 0x22 (34).

### `УстановитьВходнойКанал(НомерКанала) / SetInputChannel`
Где:<br>
- `НомерКанала` - число, номер входного канала тензодатчика.
...
Где:<br>
- `ВсеИнтерфейсы` - булево, если Истина - вернёт несколько строк со всеми IP адресами всех интерфейсов, даже виртуальных.<br>
...
```bsl
&НаКлиенте
Процедура ПодключитьсяКВесам() Экспорт

	Если НЕ КомпонентаПодключена Тогда
		УстановитьСтрокуСостоянияПодключения("Компонента не загружена", WebЦвета.Красный);
		Возврат; 
	КонецЕсли;
	
	TenzoM1CAddin.Протокол	    = Протокол;
	TenzoM1CAddin.СетевойАдрес  = СетевойАдрес;
	TenzoM1CAddin.СетевойПорт   = СетевойПорт;
	TenzoM1CAddin.СетевойРежим  = СетевойРежим;
	TenzoM1CAddin.РежимЭмуляции = РежимЭмуляции;

	TenzoM1CAddin.ГенерироватьВнешнееСобытие = ГенерироватьВнешнееСобытие;
	TenzoM1CAddin.ПосылатьНажатиеКлавиш	 = ПосылатьНажатиеКлавиш;
	TenzoM1CAddin.ПосылатьКодыНумпадКлавиш	 = ПосылатьКодыНумпадКлавиш;
	
	Если ЗначениеЗаполнено(РазделительДробнойЧасти) Тогда
		TenzoM1CAddin.РазделительДробнойЧасти = РазделительДробнойЧасти;
	КонецЕсли;
	
	ПортОткрыт = TenzoM1CAddin.Подключиться(КомПорт, КомПортСкорость, АдресУстройства);
	
	ОбновитьСтрокуСостоянияПодключения();
	
	Если ПортОткрыт Тогда
		ПодключитьОбработчикОжидания("ОбновитьВес", 0.5, Истина);
	КонецЕсли;
...
Процедура ОбновитьВес() Экспорт
	...
	Вес = TenzoM1CAddin.ПолучитьВес() / 1000;
	
	Если TenzoM1CAddin.КодОшибки<>0 Тогда
		УстановитьСтрокуСостоянияПодключения("Ошибка "+TenzoM1CAddin.КодОшибки+": "+TenzoM1CAddin.Ошибка, WebЦвета.Красный);
	КонецЕсли;
		
	Элементы.НадписьЗначениеВеса.Заголовок  = Формат(Вес, "ЧДЦ=3;  ЧН=0,000");
	Элементы.НадписьЗначениеВеса.ЦветТекста = ?(TenzoM1CAddin.ВесСтабилен, WebЦвета.Зеленый, WebЦвета.Красный);
...
&НаКлиенте
Процедура СброситьСостояниеОшибки()
	
	Если TenzoM1CAddin = Неопределено Тогда Возврат; КонецЕсли;
	
	Если TenzoM1CAddin.КодОшибки <> 0 Тогда
		TenzoM1CAddin.КодОшибки = 0;
		TenzoM1CAddin.Ошибка    = "";
	КонецЕсли;
...
&НаКлиенте
Процедура ОбновитьСтрокуСостоянияПодключения()

	СтрокаСостояния = СтрШаблон("Порт открыт: %1 Протокол: %2 IP: %3 Код ошибки: %4 Ошибка: %5",
		ПортОткрыт,
		TenzoM1CAddin.Протокол,
		TenzoM1CAddin.IP,
		TenzoM1CAddin.КодОшибки,
		TenzoM1CAddin.Ошибка);
	ЦветСостояния = ?(TenzoM1CAddin.КодОшибки=0, WebЦвета.Зеленый, WebЦвета.Красный);
	УстановитьСтрокуСостоянияПодключения(СтрокаСостояния, ЦветСостояния);
